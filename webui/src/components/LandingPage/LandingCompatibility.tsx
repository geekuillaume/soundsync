...
const PipesCanvas = ({
  currentPipes, opacity, className, audioObjectRefs,
}: {currentPipes: string[][]; opacity: number; className: string; audioObjectRefs: any}) => {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      if (lastDrawnPipes.current.currentPipes !== currentPipes) {
        opacity -= (performance.now() - lastDrawTime) / 500;
        if (opacity <= 0) {
          lastDrawnPipes.current.currentPipes = currentPipes;
        }
      } else if (opacity < 1) {
        opacity = Math.min(1, opacity + ((performance.now() - lastDrawTime) / 500));
      }
      if (ctxRef.current && canvasRef.current) {
        ...
        ctx.lineCap = 'round';
        ctx.lineWidth = 5;
        ctx.strokeStyle = `rgba(0, 96, 221, ${0.5 * opacity})`;
    ...
    <div className={classes.root}>
      <div className={classes.container}>
        ...
        <div className={classes.innerContainer}>
          <PipesCanvas currentPipes={PIPES_CONFIGURATIONS[currentPipeConfiguration]} opacity={1} className={classes.pipeCanvas} audioObjectRefs={audioObjectRefs} />

          <AudioObject name="Smartphone" icon={SmartphoneIcon} description="Smartphone" column={1} row={1} inRowCount={3} id="smartphone" refCollection={audioObjectRefs} />
          ...
          <AudioObject name="Laptop" icon={LaptopIcon} description="Laptop" column={3} row={1} inRowCount={3} rowSpan={2} id="laptopin" refCollection={audioObjectRefs} />
          ...
          <AudioObject name="Airplay Speaker" icon={AirplayIcon} description="Airplay Speaker" column={4} row={1} inRowCount={4} id="airplayout" refCollection={audioObjectRefs} />
          <AudioObject name="Laptop" icon={LaptopIcon} description="Laptop" column={4} row={2} inRowCount={4} id="laptopout" refCollection={audioObjectRefs} />
          ...
          <AudioObject name="Jack" icon={JackConnectorIcon} description="Jack" column={5} row={2} inRowCount={4} id="jack" refCollection={audioObjectRefs} />
